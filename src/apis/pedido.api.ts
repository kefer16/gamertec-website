...
export class PedidoApi {
	...
	static async Historial(ID: number): Promise<AxiosResponse> {
		try {
			...
			return await axios.get(`${PedidoCabeceraEntity.url}/historial`, config);
		} catch (err: any) {
			return Promise.reject(err);
		}
	}
	...
	static async EliminarUno(ID: number): Promise<AxiosResponse> {
		try {
			...
			return await axios.delete(`${PedidoCabeceraEntity.url}/eliminar`, config);
		} catch (err: any) {
			return Promise.reject(err);
		}
	}
	...
	static async listarPedidoUsuario(usuario_id: number): Promise<AxiosResponse> {
		try {
			const config = {
				params: {
					usuario_id,
