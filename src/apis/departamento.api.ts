...
export class DepartamentoApi {
	static async Registrar(data: DepartamentoEntity): Promise<AxiosResponse> {
		try {
			...
			return await axios.post(`${DepartamentoEntity.url}/registrar`, body, config);
		} catch (err: any) {
			console.log(err);
	...
	static async Actualizar(
		...
		try {
			...
			return await axios.put(`${DepartamentoEntity.url}/actualizar`, body, config);
		} catch (err: any) {
			console.log(err);
	...
	static async EliminarUno(ID: number): Promise<AxiosResponse> {
		try {
			...
			return await axios.delete(`${DepartamentoEntity.url}/eliminar`, config);
		} catch (err: any) {
			console.log(err);
			return Promise.reject(err);
		}
