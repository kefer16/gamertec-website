...
export const SeriesRegistro = ({
   ...
   const { privilegio, mostrarNotificacion } = useContext(
   ...
   const agregarSeries = async () => {
      ...
      await pedidoServ
         .agregarSeries(compraDetalleId, series)
         .then(() => {
            mostrarNotificacion({
               tipo: "success",
   ...
   return (
      ...
      >
         <p>
            {" "}
            {privilegio === "USU"
               ? `Numero de series en total: ${maximoOpciones}`
               : `Cantidad de series a escoger: ${maximoOpciones}`}
         </p>
         {privilegio === "USU" ? (
            <div className="card flex flex-wrap gap-2">
               {options.length <= 0 ? (
                  ...
                  options.map((item: IMultiSelectProps) => {
                     return <Chip key={item.code} label={item.name} />;
                  })
               )}
            </div>
         ) : (
